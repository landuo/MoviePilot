...
class Monitor(ConfigReloadMixin, metaclass=SingletonClass):
    ...
    def init(self):
        ...
        # worker 接管成功的目录跳过本地线程监控，失败时继续使用 Python 实现。
        ...
        for mon_dir in monitor_dirs:
            ...
            if mon_dir.storage == "local":
                if monitor_worker.make_watch_id(mon_dir) in worker_handled_dirs:
                    local_started += 1
                    logger.info(f"目录 {mon_path} 已由 mp-watcher 接管，跳过本地线程监控")
                    continue
